...
.controller('StickersController', ['$scope', '$rootScope', 'canvas', 'simpleShapes', 'history', 'settings', function($scope, $rootScope, canvas, simpleShapes, history, settings) {
    ...
    $scope.addToCanvas = function(category, index, e) {
        ...
        if (category.type === 'svg') {
            fabric.loadSVGFromURL('https://www.mundofitonline.com/shared/editor/images/stickers/'+category.name+'/'+index+'.'+category.type, function (objects, options) {
                var image = fabric.util.groupSVGElements(objects, options);
                //var image = new fabric.Image(img);
                image.name = 'sticker';
                canvas.fabric.add(image);
                image.scaleToHeight((40 / 30) * canvas.fabric.getHeight());
            ...
            fabric.util.loadImage('https://www.mundofitonline.com/shared/editor/images/stickers/'+category.name+'/'+index+'.'+category.type, function(img) {
                var image = new fabric.Image(img);
                image.name = 'sticker';
                canvas.fabric.add(image);
                image.center();
