...
class Email
{
  ...
  public function sendMessageEmail(int $perSecond = 5)
  {
    $stmt = Connect::getInstance()->query("SELECT * FROM birthday_message_queue WHERE status = 'Agendado'")->fetchAll();
    if (count($stmt) > 0) {
      foreach ($stmt as $a) {

          $this->mail->Subject = $a->subject;
          $this->mail->msgHTML($a->body);
          $this->mail->addAddress($a->recipient_email, $a->recipient_name);
          $this->mail->setFrom($a->from_email, $a->from_name);

          $this->mail->send();

          usleep(1000000 / $perSecond);
          Connect::getInstance()->exec("UPDATE birthday_message_queue SET status = 'Enviado' WHERE id = {$a->id}");
  ...
  {
    $stmt = Connect::getInstance()->query("SELECT * FROM birthday_message_queue WHERE status = 'Agendado'");
    if ($stmt->rowCount()) {
      foreach ($stmt->fetchAll() as $send) {
        ...
        if ($email->send($send->from_email, $send->from_name)) {
          usleep(1000000 / $perSecond);
          Connect::getInstance()->exec("UPDATE birthday_message_queue SET status = 'Enviado' WHERE id = {$send->id}");
        }
      }
    }
  }
