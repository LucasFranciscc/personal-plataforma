...
function urlExterna(string $path = null): string
{
    if (strpos($_SERVER['HTTP_HOST'], "localhost")) {
        if ($path) {
            return CONF_URL_TEST_EXTERNA . "/" . ($path[0] == "/" ? mb_substr($path, 1) : $path);
        }
        return CONF_URL_TEST_EXTERNA;
    }

    if ($path) {
        return CONF_URL_BASE_EXTERNA . "/" . ($path[0] == "/" ? mb_substr($path, 1) : $path);
    }

    return CONF_URL_BASE_EXTERNA;
}
...
function birth(?string $date): ?string
{
