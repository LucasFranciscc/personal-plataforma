...
class Students extends App
{
  ...
  public function students(?array $data): void
  {
    ...
    if (!empty($data["search"]) && $data["search"] != "all") {
      $search = $data["search"];
      $students = (new User())->find("name = :s", "s={$search}");
      if (!$students) {
    ...
    echo $this->view->render("views/student/students2", [
      "head" => $head,
      "search" => $search,
      "students" => $students->order("name")->limit($pager->limit())->offset($pager->offset())->order("status desc")->fetch(true),
  ...
  public function student(?array $data): void
  {
    ...
    if (!empty($data["action"]) && $data["action"] == "create") {
      ...
      $presentationCreate->user_id = $studentCreate->id;
      ...
      (new Email)->bootstrap(
        ...
        "Acesse o aplicativo nesse link <a href='https://www.app.personaledubehring.com.br/login'>Acessar App</a>  <br/><br/><br/> Seu e-mail de acesso: " . $data["email"] . "<br/> Sua senha de acesso: aluno12345",
        $data["email"],
        "{$data["name"]}"
      )->send();
